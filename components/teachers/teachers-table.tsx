...
export function TeachersTable({ teachers }: { teachers: TeacherRow[] }) {
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-[28px] border border-white/80 bg-[linear-gradient(180deg,#ffffff_0%,#fbfcfc_100%)] p-5 shadow-soft">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="space-y-1">
            <h2 className="text-xl font-bold">إدارة المعلمين</h2>
            <p className="text-sm text-slate-500">
              إضافة وتحديث وحذف ملفات المعلمين مع بحث سريع وعرض مناسب للجوال.
            </p>
          </div>
      ...
      {filteredTeachers.length === 0 ? (
        <div className="rounded-[28px] border border-dashed border-slate-300 bg-white/70 p-10 text-center shadow-soft">
          <Users2 className="mx-auto mb-4 h-10 w-10 text-slate-300" />
          <h3 className="mb-2 text-lg font-bold">لا توجد نتائج مطابقة</h3>
        ...
        <>
          ...
          <div className="grid gap-4 lg:hidden">
            {filteredTeachers.map((teacher) => (
              <div key={teacher.id} className="rounded-[28px] border border-white/80 bg-[linear-gradient(180deg,#ffffff_0%,#fbfcfc_100%)] p-5 shadow-soft">
                ...
                <div className="space-y-2 text-sm text-slate-600">
                  ...
                  <p>أضيف في: {formatDate(teacher.createdAt)}</p>
                </div>
                <div className="mt-4 flex flex-wrap gap-2">
                  <Button asChild variant="secondary" className="rounded-2xl">
                    <Link href={`/dashboard/teachers/${teacher.id}`}>عرض الملف</Link>
                  </Button>
                  <TeacherFormDialog
                    mode="edit"
                    teacher={{
